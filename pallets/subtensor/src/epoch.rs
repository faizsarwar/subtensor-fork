...
pub enum EpochRetunType<AccountId>{
    Epoch(Vec<(AccountId, u64, u64)>),
...
impl<T: Config> Pallet<T> {
    ...
    ///
    #[allow(clippy::indexing_slicing)]
    pub fn epoch(netuid: u16, incentive: Option<bool>) -> EpochRetunType<T::AccountId> {
        // Default value for incentive parameter
        let use_incentive = incentive.unwrap_or(false);

        let rao_emission = BlockEmission::<T>::get();
        ...
        if use_incentive{
            return EpochRetunType::Incentive(incentive)
        }
        ...
        EpochRetunType::<T::AccountId>::Epoch(epoch)
    }
