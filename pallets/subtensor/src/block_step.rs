        ...
        for (netuid, tempo) in <Tempo<T> as IterableStorageMap<u16, u16>>::iter() {
            ...
            let emission_tuples_this_block: Vec<(T::AccountId, u64, u64)> =
                match Self::epoch(netuid, None) {
                    epoch::EpochRetunType::Epoch(vec) => vec,
                    epoch::EpochRetunType::Incentive(_) => {
                        // Handle the case where the result is `Incentive`
                        // For this example, we'll return an empty vector or handle it as needed
                        vec![]
                    }
                };
